...
class Product
{
    ...
    // $stmt = $this->conn->prepare("SELECT * FROM `Product` WHERE is_active = 1");
    ...
    // $query = "SELECT * FROM product WHERE is_active = 1 LIMIT $starting_limit,$perPage";
    ...
    public function Category($cat){
        $query = "SELECT * FROM `Product` WHERE is_active = 1 AND product_type = :cat";
        $stmt = $this->conn->prepare($query);
...
echo "</pre>";*/
    ...
    {
        $approot = $_SERVER['DOCUMENT_ROOT'] . '/Boutique/crud/';
